...
echo "Generating VC4SM demo PKI"
...
openssl req -new -x509 -key ./keys/tls/ec-cakey.pem -subj "/C=CA/ST=ON/O=Example VC4SM Internet CA Inc.:CA Sec/OU=CA Sec" -out ./keys/tls/ec-cacert.pem

#create TLS creds
openssl ecparam -name prime256v1 -genkey -noout -out ./keys/tls/ec-key.pem
openssl req -new -key ./keys/tls/ec-key.pem -subj "/C=CA/ST=ON/O=Example Inc.:VC4SM/OU=VC4SM/CN=*.demo" -out ./keys/tls/ec-key.csr
